...
public class TConnectionManager {
    ...
    static final Map<TConnectionKey, TConnectionImplementation> ML_INSTANCES = new LinkedHashMap<TConnectionKey, TConnectionImplementation>();
    ...
    public static class TConnectionImplementation implements TConnection, Closeable {
         ...
         */
        public static final String DEFAULT_PS_ASYNC_CLIENT_PROTOCOL_CLASS = PSProtocol.AsyncProtocol.class.getName();
         ...
         * Default client protocol class name.
         */
        public static final String DEFAULT_WORKER_CLIENT_PROTOCOL_CLASS = WorkerProtocol.class.getName();
         ...
         * Default client protocol class name.
         */
        public static final String DEFAULT_MASTER_CLIENT_PROTOCOL_CLASS = MasterProtocol.class.getName();
        ...
        public TConnectionImplementation(Configuration conf, boolean managed) {
            ...
            this.closed = false;
            try {
                String clientClassName = conf.get(PS_CLIENT_PROTOCOL_CLASS, DEFAULT_PS_CLIENT_PROTOCOL_CLASS);
                this.psClientClass = (Class<? extends PSProtocol>) Class.forName(clientClassName);

                String asyncClassName = conf.get(ASYNC_PS_CLIENT_PROTOCOL_CLASS, DEFAULT_PS_ASYNC_CLIENT_PROTOCOL_CLASS);
                this.asyncPSClientClass = (Class<? extends PSProtocol.AsyncProtocol>) Class.forName(asyncClassName);

                clientClassName = conf.get(WORKER_CLIENT_PROTOCOL_CLASS, DEFAULT_WORKER_CLIENT_PROTOCOL_CLASS);
                this.workerClientClass = (Class<? extends WorkerProtocol>) Class.forName(clientClassName);

                clientClassName = conf.get(MASTER_CLIENT_PROTOCOL_CLASS, DEFAULT_MASTER_CLIENT_PROTOCOL_CLASS);
            ...
            this.maxRPCAttempts = conf.getInt(AngelConf.ML_CLIENT_RPC_MAXATTEMPTS, AngelConf.DEFAULT_ML_CLIENT_RPC_MAXATTEMPTS);
        }
        ...
        public VersionedProtocol getProtocol(final String hostname, final int port,
            ...
            Map<String, VersionedProtocol> protocols = this.servers.get(rsName);//根据ip和端口后去rpc协议
            if (protocols == null) {
                ...
                synchronized (this.connectionLock.get(lockKey)) {
                    ...
                    if (server == null) {
                        try {
                            ...
                            // this RS
                            server = MLRPC.waitForProxy(protocolClass, version, address, this.conf, this.maxRPCAttempts, this.rpcTimeout, this.rpcTimeout, addrList4Failover);
