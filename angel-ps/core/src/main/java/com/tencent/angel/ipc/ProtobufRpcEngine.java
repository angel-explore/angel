...
class ProtobufRpcEngine implements RpcEngine {
    ...
    public VersionedProtocol getProxy(Class<? extends VersionedProtocol> protocol, long clientVersion,
        ...
        if (addrList4Failover != null && addrList4Failover.size() > 1) {
            // 创建failoverHandler
            invokerHandler = new FailoverInvoker(protocol, addr, conf, factory, rpcTimeout, addrList4Failover);
        } else {
            invokerHandler = new Invoker(protocol, addr, conf, factory, rpcTimeout);
        }
        return (VersionedProtocol) Proxy.newProxyInstance(protocol.getClassLoader(), new Class[]{protocol}, invokerHandler);
    }
    ...
    static class FailoverInvoker implements InvocationHandler {
        ...
        public FailoverInvoker(Class<? extends VersionedProtocol> protocol, InetSocketAddress addr,
            ...
            List<ProtobufRpcEngine.Invoker> invokerList = new ArrayList<ProtobufRpcEngine.Invoker>();
            ...
            failoverProvider = new FailoverInvokerProvider<ProtobufRpcEngine.Invoker>(invokerList);
            currentInvoker = invokerList.get(0);
        }
            ...
            try {
                ...
                if (LOG.isDebugEnabled()) {
                    ...
                    if (LOG.isTraceEnabled()) {
                        LOG.trace("invoke Call: " + method.getName() + " " + callTime);
                    }
                }
                return val;
            } catch (Throwable e) {
