...
public class AngelYarnClient extends AngelClient {
    ...
    /**
     * rpc client to YARN resource manager
     */
    ...
    private ApplicationSubmissionContext createApplicationSubmissionContext(Configuration conf,
        ...
        capability.setMemory(conf.getInt(AngelConf.ANGEL_AM_MEMORY_GB, AngelConf.DEFAULT_ANGEL_AM_MEMORY_GB) * 1024);
        capability.setVirtualCores(conf.getInt(AngelConf.ANGEL_AM_CPU_VCORES, AngelConf.DEFAULT_ANGEL_AM_CPU_VCORES));
        ...
        localResources.put(AngelConf.ANGEL_JOB_CONF_FILE, createApplicationResource(defaultFileContext, jobConfPath, LocalResourceType.FILE));
        ...
        AngelApps.setEnvFromInputString(environment, conf.get(AngelConf.ANGEL_AM_ADMIN_USER_ENV, AngelConf.DEFAULT_ANGEL_AM_ADMIN_USER_ENV));

        AngelApps.setEnvFromInputString(environment, conf.get(AngelConf.ANGEL_AM_ENV, AngelConf.DEFAULT_ANGEL_AM_ENV));

        // Parse distributed cache
        AngelApps.setupDistributedCache(conf, localResources);

        Map<ApplicationAccessType, String> acls = new HashMap<ApplicationAccessType, String>(2);
        acls.put(ApplicationAccessType.VIEW_APP, conf.get(AngelConf.JOB_ACL_VIEW_JOB, AngelConf.DEFAULT_JOB_ACL_VIEW_JOB));
        acls.put(ApplicationAccessType.MODIFY_APP, conf.get(AngelConf.JOB_ACL_MODIFY_JOB, AngelConf.DEFAULT_JOB_ACL_MODIFY_JOB));

        // Setup ContainerLaunchContext for AM container   启动AM Container
        ...
        LOG.info("ApplicationSubmissionContext Queue name :  " + queue);
        appContext.setApplicationName(conf.get(AngelConf.ANGEL_JOB_NAME, AngelConf.DEFAULT_ANGEL_JOB_NAME));
        appContext.setCancelTokensWhenComplete(conf.getBoolean(AngelConf.JOB_CANCEL_DELEGATION_TOKEN, true));
        appContext.setAMContainerSpec(amContainer); // 设置AM Container
        appContext.setMaxAppAttempts(conf.getInt(AngelConf.ANGEL_AM_MAX_ATTEMPTS, AngelConf.DEFAULT_ANGEL_AM_MAX_ATTEMPTS));
