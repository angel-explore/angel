...
public class TaskContext {
    ...
    /**
     * 获取分配给这个Task的数据块对应的Reader，一般只在preProcess接口中使用
     *
     * @param <K> key type
     * @param <V> value type
     * @return the reader  原始数据块Reader
     * @throws ClassNotFoundException
     * @throws IOException
     * @throws InterruptedException
     */
    @SuppressWarnings("unchecked")
    public <K, V> Reader<K, V> getReader()
    ...
    /**
     * 将当前Task与任务下的所有其他Task进行同步，让他们进度一致。本方法主要用于BSP同步模型下，
     * 一般情况下并不需要应用程序显式调用它来实现同步控制（Angel在内部已经做了同步控制，内部同步过程是以矩阵为单位进行的），
     * 除非算法有这方面的特殊需求，否则不需要调用该方法
     *
     * @throws InterruptedException
     */
    ...
    /**
     * 获取某个矩阵的时钟信息。时钟信息用于进行Task之间的同步控制
     *
     * @param matrixId the matrix id
     * @return the clock
     */
